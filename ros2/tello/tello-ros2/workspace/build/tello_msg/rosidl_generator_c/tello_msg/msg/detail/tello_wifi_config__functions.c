  ...
  tello_msg__msg__TelloWifiConfig__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    ...
    tello_msg__msg__TelloWifiConfig * data =
      (tello_msg__msg__TelloWifiConfig *)realloc(output->data, allocation_size);
    if (!data) {
      return false;
    }
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!tello_msg__msg__TelloWifiConfig__init(&data[i])) {
        /* free currently allocated and return false */
        for (; i-- > output->capacity; ) {
          tello_msg__msg__TelloWifiConfig__fini(&data[i]);
        }
        free(data);
        return false;
      }
